import { z } from "zod";
import { cleanupCompetitionName, cleanupMark, extractName, formatLastname } from "./utils";

export const LastnameSchema = z.string().transform(formatLastname);
export const FullnameSchema = z.string().transform(extractName);
export const CompetitionNameSchema = z.string().transform(cleanupCompetitionName);
...
export const DateSchema = z
  .string().nullable().pipe(z.coerce.date()).transform((val) => { 
    if (!val) return null;
